...
import com.personal.hwplayer.ui.view.VideoView;
...
public class SystemPlayerActivity extends BaseActivity {
	...
	private final static int GOTO_VITAMIO_PLAYER = 2;
...
//		VioceProgressBar.setThumbOffset(13);
	...
	private boolean nextVedio(){
		...
		if(uris!=null && uris.length>currentIndext+1){
			...
			vedioInfo = vedioInfos.get(++currentVedioIndext);
			uris = vedioInfo.getUris();
			currentIndext =0;
			if(uris!=null && uris.length>currentIndext){
				player_loading.setVisibility(View.VISIBLE);
				video_view.setVideoURI(uris[currentIndext]);
	...
	private void updateProgress(){
		...
		if(tryTimes != 0 && currentPosition>0){
			tryTimes = 0;
		}
	...
	private void gotoVitamioPlayer(){
...
//				video_view.start();
	...
	private class MyOnPreparedListener implements OnPreparedListener{
		...
		public void onPrepared(MediaPlayer mp) {
			...
			isStop = false;
			updateProgress();
			mHandler.sendEmptyMessageDelayed(REMOVE_LOADING_VIEW, 200);
		}
	...
	private class MyOnErrorListener implements OnErrorListener{
		...
		public boolean onError(MediaPlayer mp, int what, int extra) {
			...
			//软解播放
			mHandler.sendEmptyMessage(GOTO_VITAMIO_PLAYER);
